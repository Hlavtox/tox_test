...
docker run --rm --volumes-from temp-ps \
       ...
       -e DISABLE_MAKE=1 \
       --workdir=/var/www/html/modules/ps_emailalerts ghcr.io/phpstan/phpstan:1.10.45-php${PHP_VERSION} \
       analyse \
       --error-format=github \
       --configuration=/var/www/html/modules/ps_emailalerts/tests/phpstan/phpstan-${PS_VERSION}.neon
